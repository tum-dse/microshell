...
for {set i 0} {$i < $cfg(n_config)} {incr i} {
    for {set j 0} {$j < $cfg(n_reg)} {incr j} {
        ...
        if {[file isdirectory "/scratch/anubhav/Coyote/examples_hw/$tmp_path/hdl"]} {
            set files_hdl [glob -nocomplain "/scratch/anubhav/Coyote/examples_hw/$tmp_path/hdl/*.svh" "/scratch/anubhav/Coyote/examples_hw/$tmp_path/hdl/*.sv" "/scratch/anubhav/Coyote/examples_hw/$tmp_path/hdl/*.v" "/scratch/anubhav/Coyote/examples_hw/$tmp_path/hdl/*.vhd"]
        ...
        # if {[file isdirectory "/scratch/anubhav/Coyote/examples_hw/$tmp_path/hdl"]} {
        #     set krnls [glob -nocomplain -tails -directory "/scratch/anubhav/Coyote/examples_hw/$tmp_path/hdl" -type d *]

        #     foreach krnl $krnls {
        #         # Copy all files
        #         file mkdir "$build_dir/$project\_config_$i/user\_c$i\_$j/hdl/ext/$krnl\_hdl"
        #         set files_hdl [glob -nocomplain "/scratch/anubhav/Coyote/examples_hw/$tmp_path/hdl/$krnl/*.svh" "/scratch/anubhav/Coyote/examples_hw/$tmp_path/hdl/$krnl/*.sv" "/scratch/anubhav/Coyote/examples_hw/$tmp_path/hdl/$krnl/*.vhd" "$hw_dir/hdl/pkg/axi_macros.svh" "$hw_dir/hdl/pkg/axi_intf.sv" "$shell_dir/hdl/lynx_pkg.sv"]
        #         if {[llength $files_hdl]} {
        ...
        #         eval $cmd
        #         set cmd "ipx::edit_ip_in_project -upgrade true -name $krnl\_c$i\_$j -directory $build_dir/$project\_config_$i/user\_c$i\_$j/test.tmp $build_dir/$project\_config_$i/user\_c$i\_$j/hdl/ext/$krnl\_hdl/component.xml; "
        ...
        # Include top
        set files_hdl [glob -nocomplain "/scratch/anubhav/Coyote/examples_hw/$tmp_path/vfpga_top.svh"]
        if {[llength $files_hdl]} {
        ...
        # IP instantiation
        if {[file exists "/scratch/anubhav/Coyote/examples_hw/$tmp_path/init_ip.tcl"]} {
            source "/scratch/anubhav/Coyote/examples_hw/$tmp_path/init_ip.tcl"
        }
