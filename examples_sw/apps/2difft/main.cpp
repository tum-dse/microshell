...
void generateTestFFTPattern(ComplexFloat* data, int size) {
   // Simple sinusoidal pattern
   for(int i = 0; i < size; i++) {
       for(int j = 0; j < size; j++) {
           int idx = i * size + j;
           data[idx].real = cos(2.0f * M_PI * (2.0f * i + 3.0f * j) / size);
           data[idx].imag = sin(2.0f * M_PI * (2.0f * i + 3.0f * j) / size);
       }
   }
}

void verifyIFFTOutput(ComplexFloat* output, int size) {
   ...
   printf("Maximum IFFT magnitude %f at position (%d,%d)\n", max_magnitude, max_i, max_j);
}
...
int main(int argc, char *argv[]) {
   ...
   if(defSize != 16) {
       throw std::runtime_error("Size must be 16x16 for this IFFT implementation");
   }
   ...
   try {
       ...
       generateTestFFTPattern(input_buffer, size);
       memset(output_buffer, 0, buffer_size);

       printComplexMatrix(input_buffer, size, "\nInput Frequency Domain Matrix");
       ...
       printComplexMatrix(output_buffer, size, "\nOutput Time Domain Matrix");
       verifyIFFTOutput(output_buffer, size);
