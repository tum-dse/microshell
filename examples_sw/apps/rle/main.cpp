...
/* Pattern types */
enum PatternType {
    PATTERN_REGULAR = 0,    // AAAABBBBCCCC... (2:1 compression)
...
void generatePattern(uint8_t* buffer, size_t size, PatternType pattern) {
    switch(pattern) {
        case PATTERN_REGULAR: // Groups of 4 identical bytes
...
CompressionStats analyzeOutput(uint8_t* buffer, size_t buffer_size, uint32_t chunks) {
    ...
    // Count RLE pairs across all chunks
...
// Print first few RLE pairs for verification
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        // Verify consistency across repetitions
        if (n_reps > 1) {
