...
constexpr auto const defSize = 256;
...
void printFFTSummary(float* output_ptr, int fft_index) {
    ...
    // Only the first 16 bins matter (the rest are the negative-frequency mirror).
    for(int i = 0; i < 16; i++) {
...
int main(int argc, char *argv[]) {
    ...
    // Each 32-point FFT consumes 32 complex floats (256 bytes); round up.
    constexpr uint32_t min_chunk_bytes = 32 * sizeof(std::complex<float>);
    ...
    uint32_t complex_size = buffer_size / sizeof(float);
    uint32_t input_size = complex_size / 2;
    uint32_t num_ffts = input_size / 32;
