...
# Parallel build script for the direct_comm_effectiveness experiment.
#
# Builds the 5 CPU-sync bitstreams (one module per vFPGA). The matching
# "direct" measurements come from the existing 5 composed-app bitstreams
# already produced by compile_hw_baseline.sh — those are the same single-
# vFPGA on-chip pipelines that feed the e2e plot, so we don't rebuild them
# here.
...
if [ -z "$1" ]; then
    ...
    echo "  Builds the 5 *_cpusync bitstreams. The composed (direct) bitstreams"
    echo "  are produced by compile_hw_baseline.sh — run that separately."
    exit 1
fi
...
# 5 CPU-sync configurations: each module on its own vFPGA.
examples=(
    "audio_processing_cpusync"
    "digital_signature_cpusync"
    "secure_storage_cpusync"
    "signed_compression_cpusync"
